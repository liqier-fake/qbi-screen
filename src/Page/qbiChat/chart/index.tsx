    ...
    <div className="flex flex-col gap-3">
      {chartRows?.map((chartRow, index) => (
        <div
          ...
          className={`${
            chartRow?.type !== "IndicatorValue" ? "flex gap-3" : ""
          }`}
        >
          {chartRow.charts.map((chart) => {
            if (
              chart.chart_type === "IndicatorValue" ||
              chart.type === "IndicatorValue"
            ) {
              return (
